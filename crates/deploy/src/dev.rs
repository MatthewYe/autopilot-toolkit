    ...
    if autopilot_dir.is_dir() {
        for entry in std::fs::read_dir(&autopilot_dir)? {
            ...
            if skill_type == SkillType::Coupled {
                ...
                // opencode command → ~/.opencode/commands/<name>.md (wrapper)
                // Generated by bootstrap.sh at install time; dev just ensures
                // the skills dir is clean for bootstrap to write into.
                ...
                // opencode: clean old flat .md (commands generated by bootstrap.sh)
                remove_project_symlink(&opencode_skills_dir.join(format!("{}.md", &name)), project_root)?;
                count += 1;
            }
        }
