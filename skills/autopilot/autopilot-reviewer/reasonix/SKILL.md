...
审查时以上游权威源为标准。读取：
- `skills/upstream/skills/engineering/tdd/SKILL.md` `## What a good test is`（测试质量标准）+ `## Anti-patterns`（反模式：implementation-coupled、tautological）
...
- **本地 `.scratch/` issue**：传入 `issue_file` 路径和权威 `contract` 文本
