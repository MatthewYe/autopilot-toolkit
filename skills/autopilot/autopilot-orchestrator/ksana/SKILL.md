...
description: Ksana autopilot loop: scan -> implement -> review -> retry via spawn agent, then global meta-review.
---
...
Execute the autopilot orchestrator workflow below. The implementer and reviewer are Ksana custom agent **roles**, installed as `~/.ksana/agents/*.toml` (auto-discovered by ksana at startup; the file's `name` field is the role name). They already contain their own methodology. Dispatch them with the `spawn_agent` tool, passing `agent_type` equal to the role name (`autopilot-implementer` or `autopilot-reviewer`).

**Do not use the `update_plan` (TODO/checklist) tool to track this workflow's phases.** The "Phase 1"/"Phase 2" headings below name workflow stages, not tool arguments. Calling `update_plan` with a `phase` field is invalid (the tool only accepts `step`/`status`) and wastes a turn. Execute the phases in order directly; the `IMPLEMENTER_REPORT`/`REVIEWER_REPORT`/`FINAL_ACCEPTANCE_REPORT` outputs are the real progress markers.
...
Spawn the Ksana implementer role via the `spawn_agent` tool:

```text
spawn_agent({
  "task_name": "implement_<feature-slug>_round<retry_count>",
  "agent_type": "autopilot-implementer",
  "message": "<task description>"
})
```

`task_name` is a lowercase/digits/underscores canonical label for the spawned agent thread. `agent_type` must be `autopilot-implementer` (the role name installed at `~/.ksana/agents/autopilot-implementer.toml`). Pass **only** the three keys above (`task_name`, `agent_type`, `message`) — `spawn_agent` rejects unknown fields. Put all task context inside the `message` string; do not invent extra top-level keys. The task description (the `message` value) must include:
...
Spawn the Ksana reviewer role via the `spawn_agent` tool:

```text
spawn_agent({
  "task_name": "review_<feature-slug>_round<retry_count>",
  "agent_type": "autopilot-reviewer",
  "message": "<task description>"
})
```

`agent_type` must be `autopilot-reviewer` (the role name installed at `~/.ksana/agents/autopilot-reviewer.toml`). Pass **only** the three keys above (`task_name`, `agent_type`, `message`) — `spawn_agent` rejects unknown fields. Put **all** task context (contract, CHANGED_FILES, SIBLING_CONTEXT, ADR/PRD references, previous REVIEWER_REPORT, etc.) inside the `message` string; do not invent extra top-level keys like file paths. The `message` value must include:
...
2. Spawn the reviewer role for an independent read-only global review:

```text
spawn_agent({
  "task_name": "meta_review",
  "agent_type": "autopilot-reviewer",
  "message": "Perform global meta-review over the whole codebase against ADRs, PRDs, and resolved issue contracts. Report Critical, Important, Suggestion, and VERDICT."
})
```
