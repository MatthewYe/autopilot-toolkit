...
Run after an autopilot session completes in Ksana. User provides the orchestrator session ID. Do not use for non-autopilot sessions.

Ksana session discovery is not finalized yet. If the user does not know the session ID, tell them session discovery is pending `TODO: ksana session export — TBD` and ask them to provide the orchestrator session ID or the relevant session trace file paths.
...
Export the orchestrator session using the Ksana session export mechanism: `TODO: ksana session export — TBD`. This must retrieve the full conversation trace including all tool calls, agent spawn records, and messages.
...
- **Subagent session IDs**: Scan Ksana agent-spawn records and child-session metadata. Track which session mapped to which agent type (implementer / reviewer) and round number
...
**If the user already specified subagent session IDs**, skip the discovery step and use the provided IDs directly. Export each subagent session using the Ksana session export mechanism: `TODO: ksana session export — TBD`. This must retrieve the full conversation trace for that subagent.
