...
Run after an autopilot session completes in Codex. User provides the orchestrator session ID. Do not use for non-autopilot sessions.

Codex session discovery is not finalized yet. If the user does not know the session ID, tell them session discovery is pending `TODO: codex session export — TBD` and ask them to provide the orchestrator session ID or the relevant session trace file paths.
...
Export the orchestrator session using the Codex session export mechanism: `TODO: codex session export — TBD`. This must retrieve the full conversation trace including all tool calls, agent spawn records, and messages.

Parse this trace to extract key metadata:
- **Issue sources**: Find paths like `.scratch/<feature>/issues/<NN-slug>/` or GitHub issue numbers in the user's initial messages
- **Subagent session IDs**: Scan Codex agent-spawn records and child-session metadata. Track which session mapped to which agent type (implementer / reviewer) and round number
...
**If the user already specified subagent session IDs**, skip the discovery step and use the provided IDs directly. Export each subagent session using the Codex session export mechanism: `TODO: codex session export — TBD`. This must retrieve the full conversation trace for that subagent.
