...
**AGENT-BRIEF**:
The normalized contract text for one issue. For canonical local tickets it is
parsed from the flat `issue_file` at
`.scratch/<feature>/issues/<NN-slug>.md`; legacy directory-shaped issues may
still carry `AGENT-BRIEF.md`. It is consumed by the implementer and reviewers.
_Avoid_: root spec, work order (the root spec is higher-level)

**Ticket artifact**:
One runnable local Markdown file with YAML `key`, `title`, `type`, `status`, and
`parent`, followed by the exact headings `What to build`, `Acceptance Criteria`,
`Blocked by`, and `Comments`. A multi-ticket `tickets.md` is not a ticket
artifact.
_Avoid_: ticket bundle, issue directory
...
One of the parallel review angles run by `autopilot-reviewer`: Behavior Alignment, TDD Discipline, Code Quality, Plan Fidelity, and (from v1.1.0) Upstream Code-Review. Each axis produces a tiered finding list; the reviewer merges them into a single verdict.
_Avoid_: review check, review dimension, review layer

**Effective agent model**:
The model used by the main agent and every autopilot child role. Custom agent
definitions must omit a model override so the host inherits the main effective
